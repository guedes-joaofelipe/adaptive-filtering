...
from adaptive_filtering.lms import SignData
...
def main(output_filepath = None):
    ...
    for ensemble in np.arange(n_ensembles):    
        ...
        n = np.sqrt(sigma_n2)*(np.random.normal(size=K)) # noite signal
        ...
        sign_data = SignData(step=mu, filter_order=filter_order, init_coef=init_coef)
        sign_data.fit(d, x)     

        W[ensemble] = sign_data.coef_vector
        MSE[ensemble] = MSE[ensemble] + np.absolute(sign_data.error_vector)**2
    ...
    if output_filepath is not None:
        ...
        fig.savefig(output_filepath + 'SignData_learning_curve.jpg')
    ...
    if output_filepath is not None:
        ...
        if not os.path.exists(output_filepath):
            os.makedirs(output_filepath)
        fig.savefig(output_filepath + 'SignData_coef_evolution.jpg', bbox_inches = 'tight')
