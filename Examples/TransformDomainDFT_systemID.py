...
#   updating.                                                                   #
#                                                                               #
#     Adaptive Algorithm used here: Transform Domain DCT                        #
#                                                                               #
#################################################################################
...
def main(output_filepath = None):
    ...
    alpha = .097        # small positive constant (0 < alpha <= 1)
    init_power = 0      # initial value for the power used to normalize the input signal
    
    ## Computing 

    # coefficient vector for each iteration and realization, w[0] = [1, 1, ..., 1]
    W = np.ones([n_ensembles, K+1, N], dtype=complex) 
    W_dcm = np.zeros([n_ensembles, K+1, N], dtype=complex) 
    ...
    for ensemble in np.arange(n_ensembles):    
        ...
        model = TransformDomain(step=mu, filter_order=filter_order, matrix='dft', init_coef=init_coef, gamma=gamma, alpha=alpha, init_power=init_power)
        model.fit(d, x)     
    ...
    # Generating plots    
    generate_learning_plots(K, N, MSE_av, MSEmin_av, W_av, w_o, output_filepath=output_filepath, algorithm='TransformDomainDCT')
