...
def main(output_filepath = None):
    ...
    for ensemble in np.arange(n_ensembles):    
        ...
        sign_error = SignError(step=mu, filter_order=filter_order, init_coef=init_coef)
        sign_error.fit(d, x)     

        W[ensemble] = sign_error.coef_vector
        MSE[ensemble] = MSE[ensemble] + np.absolute(sign_error.error_vector)**2
    ...
    if output_filepath is not None:
        ...
        fig.savefig(output_filepath + 'SignError_learning_curve.jpg')
    ...
    if output_filepath is not None:
        ...
        if not os.path.exists(output_filepath):
            os.makedirs(output_filepath)
        fig.savefig(output_filepath + 'SignError_coef_evolution.jpg', bbox_inches = 'tight')
